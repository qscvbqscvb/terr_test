...
func read(c *FortiSDKClient, method string, path string, bcomplex bool, vdomparam string) (mapTmp map[string]interface{}, err error) {
	...
	log.Printf("FOS-fortios reading response: 1")
	if fortiAPIHttpStatus404Checking(result) == true {
		mapTmp = nil
		return
	}

	log.Printf("FOS-fortios reading response: 2")
	err = fortiAPIErrorFormat(result, string(body))

	log.Printf("FOS-fortios reading response: 3")
	if err == nil {
	...
	log.Printf("FOS-fortios reading response: 4")
